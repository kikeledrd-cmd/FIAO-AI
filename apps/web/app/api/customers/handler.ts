...
import { CustomerRepository } from "@fiao/database";
...
export function createCustomersHandler(dependencies?: {
  loadBranchContext?: BranchContextLoader;
  repository?: CustomerRepository;
}) {
  const loadBranchContext = dependencies?.loadBranchContext ?? requireBranchContext;
  const repository = dependencies?.repository ?? new CustomerRepository();

  return async function customers(request: Request): Promise<NextResponse> {
    ...
    try {
      ...
      const customers = await repository.listByBranch(context.ownerId, context.branchId);
