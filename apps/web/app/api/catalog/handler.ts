...
import { CatalogRepository } from "@fiao/database";
...
export function createCatalogHandler(dependencies?: {
  loadBranchContext?: BranchContextLoader;
  repository?: CatalogRepository;
}) {
  const loadBranchContext = dependencies?.loadBranchContext ?? requireBranchContext;
  const repository = dependencies?.repository ?? new CatalogRepository();

  return async function catalog(request: Request): Promise<NextResponse> {
    ...
    try {
      ...
      const products = await repository.listActiveByBranch(context.ownerId, context.branchId);
